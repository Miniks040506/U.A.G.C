...
for (const runtime of broker.listRuntimes()) {
  ...
  console.log(`${runtime.available ? 'OK ' : 'MISS'} ${runtime.id.padEnd(12)} ${runtime.transport.padEnd(12)} caps=${caps || '-'} prerequisite=${runtime.prerequisite ?? '-'}`);
}
...
for (const profile of broker.listProfiles()) {
  if (profile.error) {
    ...
    console.log(`OK  ${profile.id}: runtime=${profile.resolved.runtime} provider=${profile.resolved.provider ?? '-'} model=${profile.resolved.model ?? 'default'} runtimeModel=${profile.resolved.runtimeModel ?? 'default'}`);
  }
}
